/*
    ...
    INSTRUCTIONS TO MARKER:
    	** TO SPECIFY THE NAME OF THE FILE YOU WISH TO ANALYSE, PLEASE NAVIGATE TO LINE 535 IN THE MAIN AND ALTER THE VARIABLE "fileToAnalyse"
    ...
    CONCERNS/ERRORS:
    	** SOMETIMES THE PROGRAM WILL DETECT A NEW LINE AT THE END OF THE FILE AND INCLUDE THAT RESULT IN THE BST		
...
BSTNode *populateBST(BSTNode *root, string file, string reservedWords[], int numReservedWords){
    ...
    while(i < fileLength){
		...
		// IF #include READ TO THE END OF LINE AND CONTINUE
		if(word.word == "include" && file[i-8] == '#'){
			while(file[i] != '\n')
				i++;
			continue;
		}
		...
		// IF #define IGNORE THE DEFINE AND GET CONSTANT (CONTINUE)
		if(word.word == "define" && file[i-7] == '#')
			continue;
		
		
		// IF using THEN READ UNTIL; AND CONTINUE
		if(word.word == "using"){
			while(file[i] != ';')
				i++;
			continue;
...
// READ RESERVED WORDS METHOD 2
int readReservedWords2(string reservedWords[], string fileToAnalyse){
...
// READ FILE METHOD 2 TO BE ANALYSED
string readFile2(string fileToAnalyse){
    ifstream fin(fileToAnalyse.c_str());
    if(!fin){
        cout << "File Error!\n\n";
        exit(-1);
    ...
    // reads past the "ENDOFLIST" sentinel
    string word;
    fin >> word;
    while(word != SENTINEL){
    	fin >> word;
	...
	// reads and stores file in the strong file
    char ch;    
    string file;
    fin >> noskipws;
    fin >> ch;	// skips the \n
...
int main(){
	...
	*/
	...
	string fileToAnalyse = "MY TESTING FILE.cpp";
	
	// getFileNameToAnalyse();
	
	// GETTING THE LIST C++ RESERVED WORDS FROM A .cpp FILE (METHOD 2)
	string reservedWords[MAX_RESERVED_WORDS];
	int numReservedWords = readReservedWords2(reservedWords, fileToAnalyse);
	// printReservedWords(reservedWords, numReservedWords);
	
  	// string file = readFile(fileToAnalyse);	// METHOD 1
  	
  	// ALTERNATIVE READ METHOD (reserved words at the top of .cpp file)
  	string file = readFile2(fileToAnalyse);
