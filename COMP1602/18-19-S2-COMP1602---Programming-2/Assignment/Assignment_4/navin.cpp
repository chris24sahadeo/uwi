...
void placeMinesOnBoard(Cell cells[MAXROWS][MAXCOLS], int numRows, int numCols, int numMines) {
	...
	srand(time(0)); //for better random number generation
	//store mines in numMines random locations on the board
	k = 0;
	while (k<numMines) { 
		//generate a random number from 0 to numRows
 		randRow = rand()%(numRows-1+1);
 		//generate a random number from 0 to numCols
 		randCol = rand()%(numCols-1+1);
 		if(!cells[randRow][randCol].hasMine) {
 		//a cell can occupy only one mine
...
int getNumber (Cell cells[MAXROWS][MAXCOLS], int numRows, int numCols, int row, int col) {
	...
	//NW cell
	if(isValidLocation(numRows,numCols,row-1,col-1)) {
		if(cells[row-1][col-1].hasMine)
		cells[row][col].number++;
	}
	//N cell
	if(isValidLocation(numRows,numCols,row-1,col)) {
		if(cells[row-1][col].hasMine)
		cells[row][col].number++;
	}
	//NE cell
	if(isValidLocation(numRows,numCols,row-1,col+1)) {
	...
	//W cell
	if(isValidLocation(numRows,numCols,row,col-1)) {
		if(cells[row][col-1].hasMine)
		cells[row][col].number++;
	}
	//E cell
	if(isValidLocation(numRows,numCols,row,col+1)) {
...
bool hasWon (Cell cells[MAXROWS][MAXCOLS], int numRows, int numCols) { 
	...
	for(i=0; i<numRows; i++) {
		for(j=0; j<numCols; j++) {
			if(!cells[i][j].hasMine && cells[i][j].isHidden)
			return false;
			if(cells[i][j].hasMine && !cells[i][j].isHidden)
			return false;			
		}
