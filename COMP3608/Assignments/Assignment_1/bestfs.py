...
# # gets a heuristic value based on __
# def h(n): 
#     pass


def bestfs(G, h, start, goal): # f(n) = h(n)

    # initialize
    open = [] # Priority queue STRUCTURE: (h((x,y)), (x,y)) ... ordered by f
        ...
        # else generate the children of X
            ...
            for child in G[X]: # https://stackoverflow.com/questions/10756427/loop-through-all-nested-dictionary-values
                ...
                if child not in [state for (f, state) in open] and child not in closed: # case 1: child is not on open or closed
                    ...
                    open.append((h[child], child)) # add child to open

                if child in [state for (f, state) in open]: # case 2: child is already on open
                    if new_g_child < g[child]: # if child was reached by a shorter path (g value only, not f!)
                        open.remove((h[child], child)) # then give the state on open the shorter path (new g value) AND HEAPIFY (done below)
                        g[child] = new_g_child
                        # f_child = g[child] + h[child] # assign child a heuristic value f(n) = g(n) + h(n)                    
                        open.append((h[child], child))
                
                if child in closed: # case 3: child is already on closed
...
print('path: {}'.format(bestfs(G, h, start, end)))
